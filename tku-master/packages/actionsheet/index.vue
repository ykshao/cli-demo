<template>
  <transition name="tk-slide-bottom">
    <div v-if="shouldRender" v-show="value" :class="b({ 'withtitle': title })">
      <div v-if="title" class="tk-hairline--top-bottom" :class="b('header')">
        <div v-text="title" />
        <icon name="close" @click="onCancel" />
      </div>
      <ul v-else class="tk-hairline--bottom">
        <li
          v-for="item in actions"
          :class="[b('item', { disabled: item.disabled }), item.className, 'tk-hairline--top']"
      ...
      <div
        v-if="cancelText"
        v-text="cancelText"
        :class="[b('cancel'), 'tk-hairline--top']"
        @click="onCancel"
      />
